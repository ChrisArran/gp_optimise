...
def estimate_positrons(params): # params = [Energy split,l0,waist]
    ...
    for i in range(Nsims):
        positron[i] = sim_positrons_angle(a0*np.sqrt(params[0])*(waist0/params[2]),
                            ...
                            duration_fwhm = duration0*1e-15, waist_fwhm = params[2]*1e-6, reject = 2*params[2]*1e-6,
...
def estimate_rr(params): # params = [Energy split,l0,waist]
    ...
    for i in range(Nsims):
        rr[i] = sim_rr_angle(a0*np.sqrt(params[0])*(waist0/params[2]), 
                            gammas[i]*(1-params[0])**gscale, gamma_spread,angle=15,l0=params[1], 
                            z_offset=z_offset[i], x_offset=x_offset[i], y_offset=y_offset[i],
...
gpo = gp_optimise.Gp_optimise(estimate_rr,dims,kernel)

# Run the thing
gpo.initialise(Ninitial=Npoints)

for i in range(9):
    gpo.optimise(Npoints,Nacq=100,explore=0.1,acq_fn='EI')

    with open('gp_rr_MC_Nsim100_jitter10um.pickle','wb') as f:
        pickle.dump(gpo,f)
